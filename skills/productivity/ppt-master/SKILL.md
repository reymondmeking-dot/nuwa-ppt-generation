...
## Quick Mode — Fast 5–10 Page Deck

Use this route when the user asks for a short deck / demo / smoke test / quick visual draft and does **not** need the full Strategist confirmation UI, source research, or template-fusion process. Quick Mode is still real PPT production — every artifact must be written and exported, not described.

1. Create or reuse a project with `project_manager.py init <name> --format ppt169`.
2. Write a compact `design_spec.md` and `spec_lock.md` with canvas, palette, font stack, image list, page rhythm, and any animation/media requirements.
3. Generate SVG pages sequentially into `<project>/svg_output/`; keep top-level semantic `<g id="...">` groups for editability and optional animation.
4. Write `notes/total.md` when speaker notes are useful.
5. Run `svg_quality_checker.py <project>` and fix every error; treat spec-lock drift warnings as missing lock entries unless the SVG is wrong.
6. Run post-processing in order: `total_md_split.py`, then `finalize_svg.py`, then `svg_to_pptx.py`.
7. Verify the final PPTX by inspecting the zip package: slide count, notes count, media files, `[Content_Types].xml`, transitions, and animation timing when enabled.

Completion criteria: exported PPTX exists under `exports/`, quality checker has 0 errors, and zip inspection confirms requested media/animation features are present.

## Dynamic GIF / Animated Image Pitfall

When a deck uses animated GIFs or the user asks for “动态图片 / 动图 / GIF”, preserve the original GIF bytes in the native PPTX export.

- Author SVG with `<image href="../images/<name>.gif" .../>`.
- Export native PPTX with `--no-image-optimize`; otherwise Pillow-based optimization may transcode GIF to PNG/JPEG and silently remove animation.
- After export, inspect the PPTX zip and verify all of:
  - `ppt/media/*.gif` exists
  - `[Content_Types].xml` contains `image/gif` or `Extension="gif"`
  - slide relationship files reference `.gif`
- If any check fails, re-export with `--no-image-optimize` before reporting success.
- For a quick automated check, run `python ${SKILL_DIR}/scripts/verify_pptx_export.py <exports/*.pptx> [--require-gif]`.

## Windows / Pillow Path Pitfall

On Windows Hermes sessions, Python path pollution can make the project venv import a broken Pillow from Hermes' own venv. Symptom: `ImportError: cannot import name '_imaging' from 'PIL'` or `Error: PIL (Pillow) is required` even after installing Pillow.

Fix by forcing the project venv site-packages first for image/finalize/export commands:

```bash
PYTHONPATH='D:/AI/ppt-master-sparse/.venv/Lib/site-packages' python <script> ...
```

Use the actual repo path if different. Verify with:

```bash
python -c 'from PIL import Image; import PIL; print(PIL.__version__, Image.__file__)'
```

## macOS Compatibility Notes

The core PPT Master pipeline is Python/SVG/PPTX based and is expected to run on macOS with a normal virtualenv. Use this section when the host is macOS or the user asks for Mac compatibility.

**Recommended setup**:

```bash
cd ~/AI/ppt-master-sparse   # or the actual repo path
python3 -m venv .venv
...
Inside the venv, prefer `python` for consistency; outside the venv, macOS commonly uses `python3`.

**Optional SVG/PNG preview dependencies** — only needed when compatibility raster preview / CairoSVG rendering is required, not for the canonical native PPTX export:
...
**Font stack**: for Chinese decks on macOS, prefer:

```text
"PingFang SC", "Helvetica Neue", Arial, sans-serif
```

Use `"Microsoft YaHei", Arial, sans-serif` only when the output target is Windows/Office and the font is available. Keep the final fallback PPT-safe (`Arial`, `Helvetica`, or a generic sans-serif).

**Animated GIF rule is unchanged on macOS**: when SVG pages reference `.gif` files, export native PPTX with `--no-image-optimize` and verify `ppt/media/*.gif` exists in the package.

**PowerPoint / Keynote compatibility**:

- PowerPoint for Mac should preserve native PPTX slides, speaker notes, GIF media, transitions, and most OOXML object entrance animations.
- Keynote can open the PPTX, but may downgrade some PowerPoint-specific transitions or object animations. If the target presenter uses Keynote, do a manual visual playback check before final delivery.

**macOS quick export example with GIF + animation**:

```bash
source .venv/bin/activate
python skills/ppt-master/scripts/svg_quality_checker.py projects/<project>
python skills/ppt-master/scripts/total_md_split.py projects/<project>
python skills/ppt-master/scripts/finalize_svg.py projects/<project>
python skills/ppt-master/scripts/svg_to_pptx.py projects/<project> \
...
- `python -c 'from PIL import Image; import PIL; print(PIL.__version__, Image.__file__)'` points inside the project `.venv`.
- `svg_quality_checker.py` exits with 0 errors.
- Final PPTX exists under `exports/`.
- For GIF decks: zip inspection confirms `ppt/media/*.gif`, `image/gif` content type, and slide relationship references to `.gif`.
- If final playback target is Keynote, open and play through manually because Keynote may simplify some effects.
...
> when the deck must retain original image bytes, and **MUST add it for animated
> GIFs** so the GIF is not transcoded to a static PNG/JPEG. Use
...
>
> **Post-export verification** — after every export, verify with:
> ```bash
> python ${SKILL_DIR}/scripts/verify_pptx_export.py <exports/*.pptx> [--require-gif]
> ```
> This inspects zip internals (slides, notes, media, transitions, animations)
> so you catch media-optimisation regressions or missing slides before reporting success.
